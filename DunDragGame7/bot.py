...
import random
from game_logic import random_encounter
...
load_dotenv()  # Завантажує файл .env
...
def deco(func):
    def wrapper(call):
        # Виведення повідомлення перед виконанням основної функції
        bot.send_message(call.message.chat.id, "Гра починається! Тепер ви можете вибрати шлях.")

        # Викликаємо оригінальну функцію без змін
        return func(call)

    return wrapper

# Приклад функції для старту гри
